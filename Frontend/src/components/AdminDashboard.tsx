...
const AdminDashboard = () => {
  ...
  const unresolvedComplaints = complaints.filter(report => report.status !== 'resolved');
  // Show loading state
  if (loading) {
    return (
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      const updatedReportRes = await api.get(`/api/reports/${viewingReport}`);
      const updatedReport = updatedReportRes.data.data;
      setComplaints((prev) =>
        prev.map((r) => (r._id === viewingReport ? updatedReport : r))
      );
